...
if __name__ == '__main__':
    ...
    for celltype in ['enterocyte', 'spermatocyte']:
        ...
        for specie in ['HS', 'MM', 'DM']:
            ...
            rFPKMfile = "../2-core_genes/results/{specie:s}-FPKM-{celltype:s}.csv.gz".format(celltype=celltype, specie=specie)
            df = pd.read_csv(rFPKMfile, index_col='id_gene', usecols=['id_string', 'id_gene', 'gene', 'FPKM', 'TPM', celltype, 'biotype'])
            # Remove Duplicates
            df = df.loc[~df.index.duplicated(), :]
            # Only TPM > log(2) & CellType
            df = df.loc[((df['TPM'] >= minLogTPM) & (df[celltype] == True)), :]
    ...
    df_stat = pd.DataFrame(r, columns=['Species', 'Celltype', '# FPKM (logTPM>=1)', '# PC', '# w/ String ID', '# w/ StringID & PC'])
