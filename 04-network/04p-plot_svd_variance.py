...
if __name__ == '__main__':
    ...
    #
    wIMGFile = 'images/svd/{celltype:s}/img-svd-variance-{celltype:s}-{network:s}-{threshold:s}.pdf'.format(celltype=celltype, network=network, threshold=threshold_str)
    variances = []
    ...
    for layer, ax in zip(['HS', 'MM', 'DM'], [ax_hs, ax_mm, ax_dm]):
        print('Plotting SVD Variance for {celltype:s}-{network:s}-{threshold:s}-{layer:s}'.format(celltype=celltype, network=network, threshold=threshold_str, layer=layer))
