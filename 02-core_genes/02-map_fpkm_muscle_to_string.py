...
# Date: Jul 05, 2019
#
# Description: Maps FPKM Muscle genes to String-DB
#
import tarfile
import math
import pandas as pd
...
if __name__ == '__main__':
    ...
    # Load FPKM
    tar_HS = tarfile.open('../01-diff-gene-exp/data/FPKM/HS/HS_Muscle.tar.gz', "r:gz")
    df_HS = pd.concat(
        [
            pd.read_csv(tar_HS.extractfile('HS_Muscle_6T1A_Rep1.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_HS.extractfile('HS_Muscle_10T1A_Rep2.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_HS.extractfile('HS_Muscle_17T1A_Rep3.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_HS.extractfile('HS_Muscle_29T1A_Rep4.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_HS.extractfile('HS_Muscle_30T1A_Rep5.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_HS.extractfile('HS_Muscle_31T1A_Rep6.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
        ],
    ...
    # Index Rows/Cols
    maskcols = ['id_string', 'gene', 'muscle', 'FPKM', 'TPM', 'biotype']
    df_HS = df_HS.loc[:, maskcols]
    # To CSV
    df_HS.to_csv('results/HS-FPKM-muscle.csv.gz')
    ...
    # Load FPKM
    tar_MM = tarfile.open('../01-diff-gene-exp/data/FPKM/MM/MM_Muscle.tar.gz', "r:gz")
    df_MM = pd.concat(
        [
            pd.read_csv(tar_MM.extractfile('MM_Muscle_Rep1.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_MM.extractfile('MM_Muscle_Rep2.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_MM.extractfile('MM_Muscle_Rep3.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_MM.extractfile('MM_Muscle_Rep4.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_MM.extractfile('MM_Muscle_Rep5.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_MM.extractfile('MM_Muscle_Rep6.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
        ],
        axis='index', sort=True)
    df_MM = df_MM.groupby(df_MM.index).mean()
    df_MM.index.name = 'id_gene'
    df_MM.index = df_MM.index.map(lambda x: x.split('.')[0])
    ...
    # To CSV
    df_MM.to_csv('results/MM-FPKM-muscle.csv.gz')
    ...
    # Load FPKM
    tar_DM = tarfile.open('../01-diff-gene-exp/data/FPKM/DM/DM_Muscle.tar.gz', "r:gz")
    df_DM = pd.concat(
        [
            pd.read_csv(tar_DM.extractfile('DM_Muscle_Rep1.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM'])
        ],
        axis='index', sort=True)
    ...
    # To CSV
    df_DM.to_csv('results/DM-FPKM-muscle.csv.gz')
