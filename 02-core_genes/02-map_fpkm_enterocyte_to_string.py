...
# Date: Jul 05, 2019
#
# Description: Maps FPKM Enterocyte genes to String-DB
#
import tarfile
import math
import pandas as pd
...
if __name__ == '__main__':
    ...
    df_HS = pd.concat(
        [
            ...
            pd.read_csv(tar_HS.extractfile('HS_Enterocyte_Rep2.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_HS.extractfile('HS_Enterocyte_Rep3.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
        ],
    ...
    df_HS['enterocyte'] = True
    df_HS['biotype'] = df_HS_G['Gene type']
    # Index Rows/Cols
    maskcols = ['id_string', 'gene', 'enterocyte', 'FPKM', 'TPM', 'biotype']
    df_HS = df_HS.loc[:, maskcols]
    # To CSV
    df_HS.to_csv('results/HS-FPKM-enterocyte.csv.gz')
    ...
    df_MM = pd.concat(
        [
            ...
            pd.read_csv(tar_MM.extractfile('MM_Enterocyte_Rep2.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_MM.extractfile('MM_Enterocyte_Rep3.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
        ],
        axis='index', sort=True)
    ...
    df_MM['enterocyte'] = True
    df_MM['biotype'] = df_MM_G['Gene type']
    # Index Rows/Cols
    maskcols = ['id_string', 'gene', 'enterocyte', 'FPKM', 'TPM', 'biotype']
    df_MM = df_MM.loc[:, maskcols]
    # To CSV
    df_MM.to_csv('results/MM-FPKM-enterocyte.csv.gz')
    ...
    df_DM = pd.concat(
        [
            ...
            pd.read_csv(tar_DM.extractfile('DM_Enterocyte_Rep2.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM'])
        ],
        axis='index', sort=True)
    ...
    df_DM['enterocyte'] = True
    df_DM['biotype'] = df_DM_G['Gene type']
    # Index Rows/Cols
    maskcols = ['id_string', 'gene', 'enterocyte', 'FPKM', 'TPM', 'biotype']
    df_DM = df_DM.loc[:, maskcols]
    # To CSV
    df_DM.to_csv('results/DM-FPKM-enterocyte.csv.gz')
