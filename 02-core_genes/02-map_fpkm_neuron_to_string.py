...
# Date: Jul 05, 2019
#
# Description: Maps FPKM Neuron genes to String-DB
#
import tarfile
import math
import pandas as pd
...
if __name__ == '__main__':
    ...
    # Load FPKM
    tar_HS = tarfile.open('../01-diff-gene-exp/data/FPKM/HS/HS_Neuron.tar.gz', "r:gz")
    df_HS = pd.concat(
        [
            pd.read_csv(tar_HS.extractfile('HS_Neuron_Rep1.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_HS.extractfile('HS_Neuron_Rep2.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_HS.extractfile('HS_Neuron_Rep3.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_HS.extractfile('HS_Neuron_Rep4.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
        ],
    ...
    # Index Rows/Cols
    maskcols = ['id_string', 'gene', 'neuron', 'FPKM', 'TPM', 'biotype']
    df_HS = df_HS.loc[:, maskcols]
    # To CSV
    df_HS.to_csv('results/HS-FPKM-neuron.csv.gz')
    ...
    # Load FPKM
    tar_MM = tarfile.open('../01-diff-gene-exp/data/FPKM/MM/MM_Neuron.tar.gz', "r:gz")
    df_MM = pd.concat(
        [
            pd.read_csv(tar_MM.extractfile('MM_Neuron_Rep1.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_MM.extractfile('MM_Neuron_Rep2.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_MM.extractfile('MM_Neuron_Rep3.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
        ],
        axis='index', sort=True)
    ...
    # To CSV
    df_MM.to_csv('results/MM-FPKM-neuron.csv.gz')
    ...
    # Load FPKM
    tar_DM = tarfile.open('../01-diff-gene-exp/data/FPKM/DM/DM_Neuron.tar.gz', "r:gz")
    df_DM = pd.concat(
        [
            pd.read_csv(tar_DM.extractfile('DM_Neuron_Rep1.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_DM.extractfile('DM_Neuron_Rep2.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_DM.extractfile('DM_Neuron_Rep3.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_DM.extractfile('DM_Neuron_Rep4.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_DM.extractfile('DM_Neuron_Rep5.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_DM.extractfile('DM_Neuron_Rep6.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
        ],
        axis='index', sort=True)
    ...
    # To CSV
    df_DM.to_csv('results/DM-FPKM-neuron.csv.gz')
