...
# Date: Jul 05, 2019
#
# Description: Maps FPKM Spermatocyte genes to String-DB
#
import tarfile
import math
import pandas as pd
...
if __name__ == '__main__':
    ...
    # Load FPKM
    tar_HS = tarfile.open('../01-diff-gene-exp/data/FPKM/HS/HS_Spermatocyte.tar.gz', "r:gz")
    df_HS = pd.concat(
        [
            pd.read_csv(tar_HS.extractfile('HS_Spermatocyte_rep1.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_HS.extractfile('HS_Spermatocyte_rep2.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_HS.extractfile('HS_Spermatocyte_rep3.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_HS.extractfile('HS_Spermatocyte_rep4.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_HS.extractfile('HS_Spermatocyte_rep5.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_HS.extractfile('HS_Spermatocyte_rep6.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_HS.extractfile('HS_Spermatocyte_rep7.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_HS.extractfile('HS_Spermatocyte_rep8.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_HS.extractfile('HS_Spermatocyte_rep9.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_HS.extractfile('HS_Spermatocyte_rep10.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_HS.extractfile('HS_Spermatocyte_rep11.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_HS.extractfile('HS_Spermatocyte_rep12.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
        ],
    ...
    df_HS['spermatocyte'] = True
    df_HS['biotype'] = df_HS_G['Gene type']
    # Index Rows/Cols
    maskcols = ['id_string', 'gene', 'spermatocyte', 'FPKM', 'TPM', 'biotype']
    df_HS = df_HS.loc[:, maskcols]
    # To CSV
    df_HS.to_csv('results/HS-FPKM-spermatocyte.csv.gz')
    ...
    # Load FPKM
    tar_MM = tarfile.open('../01-diff-gene-exp/data/FPKM/MM/MM_Spermatocyte.tar.gz', "r:gz")
    df_MM = pd.concat(
        [
            pd.read_csv(tar_MM.extractfile('MM_Spermatocyte_rep1.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_MM.extractfile('MM_Spermatocyte_rep2.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
            pd.read_csv(tar_MM.extractfile('MM_Spermatocyte_rep3.txt'), sep='\t', index_col='Gene ID', usecols=['Gene ID', 'FPKM', 'TPM']),
        ],
        axis='index', sort=True)
    ...
    df_MM['spermatocyte'] = True
    df_MM['biotype'] = df_MM_G['Gene type']
    # Index Rows/Cols
    maskcols = ['id_string', 'gene', 'spermatocyte', 'FPKM', 'TPM', 'biotype']
    df_MM = df_MM.loc[:, maskcols]
    # To CSV
    df_MM.to_csv('results/MM-FPKM-spermatocyte.csv.gz')
    ...
    # Load FPKM
    tar_DM = tarfile.open('../01-diff-gene-exp/data/FPKM/DM/DM_Middle.tar.gz', "r:gz")
    df_DM = pd.concat(
        [
    ...
    df_DM['spermatocyte'] = True  # Middle
    df_DM['biotype'] = df_DM_G['Gene type']
    # Index Rows/Cols
    maskcols = ['id_string', 'gene', 'spermatocyte', 'FPKM', 'TPM', 'biotype']
    df_DM = df_DM.loc[:, maskcols]
    # To CSV
    df_DM.to_csv('results/DM-FPKM-spermatocyte.csv.gz')
