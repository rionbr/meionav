...
if __name__ == '__main__':
    ...
    for maxFDR in [0.01, 0.05]:
        ...
        # Load Species Files
        rCSVFileDM = 'results/{pipeline:s}/DM/genes_DM-{maxFDR:s}.csv'.format(pipeline=pipeline, maxFDR=maxFDR_str)
        df_DM = pd.read_csv(rCSVFileDM, index_col='id_string')
        rCSVFileHS = "results/{pipeline:s}/HS/genes_HS-{maxFDR:s}.csv".format(pipeline=pipeline, maxFDR=maxFDR_str)
        df_HS = pd.read_csv(rCSVFileHS, index_col='id_string')
        rCSVFileMM = "results/{pipeline:s}/MM/genes_MM-{maxFDR:s}.csv".format(pipeline=pipeline, maxFDR=maxFDR_str)
        ...
        string_DM_up = df_DM.loc[(df_DM['UpMiddle_vs_Apical'] == True), :].index.tolist()
        ...
        string_DM_down = df_DM.loc[(df_DM['DownMiddle_vs_Basal'] == True), :].index.tolist()
