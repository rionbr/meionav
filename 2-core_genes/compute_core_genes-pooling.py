...
if __name__ == '__main__':
    ...
    for maxFDR in [0.01, 0.05]:
        ...
        # Load Species Files
        rCSVFileDM = 'results/{pipeline:s}/DM/genes_DM.csv'.format(pipeline=pipeline)  # changes here
        df_DM = pd.read_csv(rCSVFileDM, index_col='id_string')
        rCSVFileHS = "results/{pipeline:s}/HS/genes_HS-{maxFDR:s}.csv".format(pipeline='conserved', maxFDR=maxFDR_str)  # changes here
        df_HS = pd.read_csv(rCSVFileHS, index_col='id_string')
        rCSVFileMM = "results/{pipeline:s}/MM/genes_MM-{maxFDR:s}.csv".format(pipeline='conserved', maxFDR=maxFDR_str)  # changes here
        ...
        string_HS_up = df_HS.loc[(df_HS['UpCyte_vs_Gonia'] == True), :].index.tolist()
        string_MM_up = df_MM.loc[(df_MM['UpCyte_vs_Gonia'] == True), :].index.tolist()
        string_DM_ma = df_DM.loc[(df_DM['Middle_vs_Apical'] == True), :].index.tolist()  # changes here

        # Only Down Regulated
        string_HS_down = df_HS.loc[(df_HS['DownCyte_vs_Tid'] == True), :].index.tolist()
        string_MM_down = df_MM.loc[(df_MM['DownCyte_vs_Tid'] == True), :].index.tolist()
        string_DM_mb = df_DM.loc[(df_DM['Middle_vs_Basal'] == True), :].index.tolist()  # changes here
        ...
        def selected_sperm_genes(genes,
                ...
                string_HS_up, string_MM_up, string_DM_ma,
                string_HS_down, string_MM_down, string_DM_mb):
            ...
            DM_ma = [True if gene in string_DM_ma else False for gene in spermgenes_DM]
            ...
            DM_mb = [True if gene in string_DM_mb else False for gene in spermgenes_DM]
            return pd.Series({
                'id_string_HS': spermgenes_HS, 'id_string_MM': spermgenes_MM, 'id_string_DM': spermgenes_DM,
                'HS_up': HS_up, 'MM_up': MM_up, 'DM_ma': DM_ma,
                'HS_down': HS_down, 'MM_down': MM_down, 'DM_mb': DM_mb,
            })
        ...
        print("> Exporting")
        # Export this too
        wCSVFileSG = 'results/{pipeline:s}/meiotic_genes/all_meiotic_genes-{maxFDR:s}.csv'.format(pipeline=pipeline, maxFDR=maxFDR_str)
        ...
        wCSVFileSSG3 = 'results/{pipeline:s}/core_meiotic_genes/core_meiotic_genes-{maxFDR:s}.csv'.format(pipeline=pipeline, maxFDR=maxFDR_str)
