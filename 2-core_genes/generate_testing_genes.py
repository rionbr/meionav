...
if __name__ == '__main__':
    ...
    for maxFDR in [0.05, 0.01]:
        ...
        rCSVFile = 'results/core_meiotic_genes/core_meiotic_genes-{maxFDR:s}.csv'.format(maxFDR=maxFDR_str)
        ...
        dfU = dfU.reset_index(level=['gene_DM', 'DM_up', 'DM_down', 'biotype_DM']).drop_duplicates(keep='first')
        ldfDM.append(dfU)

        wCSVfile = 'results/DM/core_DM_meiotic_genes-{maxFDR:s}.csv'.format(maxFDR=maxFDR_str)
        dfU.to_csv(wCSVfile)
        ...
        wCSVfile = 'results/HS/core_HS_meiotic_genes-{maxFDR:s}.csv'.format(maxFDR=maxFDR_str)
        dfU.to_csv(wCSVfile)
        ...
        wCSVfile = 'results/MM/core_MM_meiotic_genes-{maxFDR:s}.csv'.format(maxFDR=maxFDR_str)
        dfU.to_csv(wCSVfile)
    ...
    dfDM.to_csv('results/DM/core_DM_meiotic_genes.csv')

    dfHS = ldfHS[0]
    dfHS1 = ldfHS[1]
    dfHS['FDR<=0.05'] = True
    dfHS['FDR<=0.01'] = dfHS.index.isin(dfHS1.index)
    dfHS.to_csv('results/HS/core_HS_meiotic_genes.csv')

    dfMM = ldfMM[0]
    dfMM1 = ldfMM[1]
    dfMM['FDR<=0.05'] = True
    dfMM['FDR<=0.01'] = dfMM.index.isin(dfMM1.index)
    dfMM.to_csv('results/MM/core_MM_meiotic_genes.csv')
