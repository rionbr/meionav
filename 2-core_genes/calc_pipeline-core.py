...
if __name__ == '__main__':
    ...
    # Export
    print("> Exporting")
    wCSVFile = 'results/pipeline-{pipeline:s}/meta_meiotic_genes.csv'.format(pipeline=pipeline)
    ensurePathExists(wCSVFile)
    df.to_csv(wCSVFile)
    
    # HS
    wCSVFileHS = 'results/pipeline-{pipeline:s}/HS_meiotic_genes.csv'.format(pipeline=pipeline)
    ensurePathExists(wCSVFileHS)
    df_HS.to_csv(wCSVFileHS)

    # MM
    wCSVFileMM = 'results/pipeline-{pipeline:s}/MM_meiotic_genes.csv'.format(pipeline=pipeline)
    ensurePathExists(wCSVFileMM)
    df_MM.to_csv(wCSVFileMM)

    wCSVFileDM = 'results/pipeline-{pipeline:s}/DM_meiotic_genes.csv'.format(pipeline=pipeline)
