...
if __name__ == '__main__':
    ...
    wMLTFile = 'results/clf-conserved-without-exp/{layer:s}/ml-{celltype:s}-{layer:s}-auc.csv.gz'.format(celltype=celltype, layer=layer)
    ensurePathExists(wMLTFile)
    dfT.to_csv(wMLTFile, encoding='utf-8', index=False)

    print('> Features')
    dfF = pd.DataFrame(f, columns=['clf', 'fold', 'feature', 'coef'])
    wMLFFile = 'results/clf-conserved-without-exp/{layer:s}/ml-{celltype:s}-{layer:s}-features.csv.gz'.format(celltype=celltype, layer=layer)
    ...
    wMLCFile = 'results/clf-conserved-without-exp/{layer:s}/ml-{celltype:s}-{layer:s}-clf.csv.gz'.format(celltype=celltype, layer=layer)
