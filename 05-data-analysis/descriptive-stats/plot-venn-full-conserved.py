...
if __name__ == '__main__':
    ...
    rG_full_file_gpickle = path_net + 'net-{celltype:s}-{network:s}.gpickle'.format(celltype=celltype, network=network)
    ...
    for ax, layer in zip(axes, ['HS', 'MM', 'DM']):
        ...
        Gtf = dict_data[layer]['graph_full']
        ...
        # Plots
        v = venn2(ax=ax,
                  subsets=[set_full_genes, set_conserved_genes],
                  set_labels=('Complete', 'Conserved'),
                  #set_colors=[],
                  alpha=1.0,
    ...
    file = img_path + 'img-venn-{celltype:s}-full-vs-conserved-{threshold:s}.pdf'.format(celltype=celltype, threshold=threshold_str)
