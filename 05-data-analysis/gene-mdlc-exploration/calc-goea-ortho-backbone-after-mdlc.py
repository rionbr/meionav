...
# Date: Aug 26, 2021
#
# Description: Calculates GO enrichment analysis nodes (based on edges) disrupted in the mdlc mutant
#
#
import pandas as pd
...
if __name__ == '__main__':
    ...
    wCSVFile = 'results/goea/goea-ortho-backbone-{layer:s}-mdlc.csv.gz'.format(layer=layer)
    ...
    if layer == 'DM':
        ...
        #
        is_mdlc_dge = set(dfM.index.tolist())

    #
    is_ortho_metric_edges = [(i, j) for i, j, d in OB.edges(data=True) if (d.get('is_metric_ortho') == is_metric_ortho_string) and ((i in is_mdlc_dge) or (j in is_mdlc_dge))]
