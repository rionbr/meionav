...
if __name__ == '__main__':
    ...
    df_DM = pd.read_csv("results/DM-DGE-mdlc_vs_control-SELECTED-string.csv", index_col='id_gene', usecols=['id_gene', 'id_string', 'gene', 'logFC', 'logCPM', 'FDR'])
    ...
    set_id_strings = set.union(set_HS_id_strings, set(), set_DM_id_strings)
    ...
    dfM = df_Egg.apply(select_by_gene_and_separate_by_species, args=(set_HS_id_strings, set(), set_DM_id_strings))
    ...
    ##
    print('Exporting')
    wGfile_gpickle = 'results/net-{celltype:s}-{network:s}.gpickle'.format(celltype=celltype, network=network)
