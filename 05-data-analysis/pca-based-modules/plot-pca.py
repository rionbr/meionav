...
def plot_pca(celltype='spermatocyte', network='thr', threhsold=0.5, layer='DM'):
    """ Plot PCA """
    threshold_str = str(threshold).replace('.', 'p')
    #
    data_cell = data_cells[celltype]
    modules = data_cell['modules-pca']['modules'][layer]
    ...
    rPCAFile = '../../04-network/results/pca/{celltype:s}/pca-{celltype:s}-{network:s}-{threshold:s}-{layer:s}-dim.csv.gz'.format(celltype=celltype, network=network, threshold=threshold_str, layer=layer)
    ...
    wIMGFile = 'images/{celltype:s}/img-pca-{celltype:s}-{network:s}-{threshold:s}-{layer:s}.pdf'.format(celltype=celltype, network=network, threshold=threshold_str, layer=layer)
    ...
    title = 'PCA on {celltype:s}-{network:s}-{threshold:s}-{layer:s} network'.format(celltype=celltype, network=network, threshold=str(threshold), layer=layer)
    fig.suptitle(title)
    ...
    for dim, ax in zip(range(1, 11), [ax2, ax3, ax4, ax5, ax6, ax7, ax8, ax9]):
        ...
        """
        if layer == 'DM':
            pca_colors = df_pca['core'].map(lambda x: '#ff9896' if x == True else '#7f7f7f') # df_pca['color'].tolist()
            pca_edgecolors = df_pca['core'].map(lambda x: '#d62728' if x == True else '#c7c7c7') # df_pca['color'].tolist()
        else:
            pca_colors = df_pca['mammals'].map(lambda x: '#ff9896' if x == True else '#7f7f7f') # df_pca['color'].tolist()
            pca_edgecolors = df_pca['mammals'].map(lambda x: '#d62728' if x == True else '#c7c7c7') # df_pca['color'].tolist()
        """
        pca_colors = '#7f7f7f'
        ...
        for module in modules:
            ...
            if (dim == xcomp) and ((dim + 1) == ycomp):
                ...
                ax.annotate(s=name, xy=(xtext, ytext), fontsize='xx-small', zorder=8, fontweight='bold', ha=ha, va='center')
            else:
                continue
...
if __name__ == '__main__':
    ...
    #plot_pca(celltype, network, threshold, layer)
    #asd
