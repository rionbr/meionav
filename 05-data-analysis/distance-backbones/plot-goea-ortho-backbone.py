...
def plot_goea_top5(celltype='spermatocyte', layer='DM'):

    rCSVFile = 'results/goea/goea-ortho-backbone-{layer:s}.csv.gz'.format(layer=layer)
    ...
    # Top n
    dft10 = df.iloc[:5, :].sort_values('1-log(p)', ascending=True)
    sl = 75  # string slice
    ...
    # P-values
    #title = 'GOEA-{specie:s} ortholog backbone'.format(specie=specie)
    title = 'OrthoBackbone genes'
    ...
    for bar, name in zip(bp.patches, dft10['name'].tolist()):
        ...
        tx = bx + (0.01 * maxx)
        ty = (by + (bh / 2))
        ax.text(x=tx, y=ty, s=name, ha='left', va='center', fontsize=10, zorder=5, color='black')
    #
    ...
    ax.set_ylim(-0.7, (5 - 0.3))
    ax.grid(axis='x', zorder=1)

    # Top 5
    plt.subplots_adjust(left=0.21, right=0.97, bottom=0.28, top=0.86)
    #plt.tight_layout()
    #
    wIMGFile = 'images/goea-bars-ortho-backbone/img-goea-bars-{celltype:s}-{layer:s}-ortho-backbone-top5.pdf'.format(celltype=celltype, layer=layer)
    ensurePathExists(wIMGFile)
    plt.savefig(wIMGFile, dpi=300, bbox_inches=None, pad_inches=0.0)
    plt.close()


def plot_goea_top10(celltype='spermatocyte', layer='DM'):
    ...
    for bar, name in zip(bp.patches, dft10['name'].tolist()):
        ...
        tx = bx + (0.01 * maxx)
        ty = (by + (bh / 2))
        ax.text(x=tx, y=ty, s=name, ha='left', va='center', fontsize='x-small', zorder=5, color='white')
    #
    ax.axvline(x=(1 - math.log(0.01)), color='#666666', ls='dotted')
    ax.axvline(x=(1 - math.log(0.05)), color='#c7c7c7', ls='dashed')
    ax.set_yticks(ind)
    ...
    #plt.tight_layout()
    #
    wIMGFile = 'images/goea-bars-ortho-backbone/img-goea-bars-{celltype:s}-{layer:s}-ortho-backbone-top10.pdf'.format(celltype=celltype, layer=layer)
    ensurePathExists(wIMGFile)
    plt.savefig(wIMGFile, dpi=300, bbox_inches=None, pad_inches=0.0)
    plt.close()
...
def plot_wordcloud(celltype='spermatocyte', layer='DM'):
    ...
    title = 'GOEA-{specie:s} ortholog backbone'.format(specie=specie, celltype=celltype_str)
    ax.set_title(title)
    #
    ...
    #
    wIMGFile = 'images/goea-wordcloud-ortho-backbone/img-goea-wc-{celltype:s}-{layer:s}-ortho-backbone.pdf'.format(celltype=celltype, layer=layer)
    ensurePathExists(wIMGFile)
    plt.savefig(wIMGFile, dpi=300, bbox_inches=None, pad_inches=0.0)
    plt.close()


def plot_combined_wordcloud(celltype='spermatocyte'):
    ...
    for layer in ['HS', 'MM', 'DM']:
        ...
        df = df.iloc[:10, :].sort_values('1-log(p)', ascending=True)
        #
        ldfs.append(df)
    #
    df = pd.concat(ldfs)
    ...
    #
    english_stopwords = stopwords.words('english')
    print('Plotting GOEA Wordcloud: {celltype:s} (combined)'.format(celltype=celltype, layer=layer))
    ...
    title = 'GOEA-Ortholog backbone (all species)'.format(specie=specie, celltype=celltype_str)
    ax.set_title(title)
    #
    wp = ax.imshow(wordcloud, interpolation='bilinear')
    #
    ...
    plt.subplots_adjust(left=0.03, right=0.97, bottom=0.17, top=0.89)
    #
    wIMGFile = 'images/goea-wordcloud-ortho-backbone/img-goea-wc-{celltype:s}-ortho-backbone-combined.pdf'.format(celltype=celltype, layer=layer)
    ensurePathExists(wIMGFile)
    plt.savefig(wIMGFile, dpi=300, bbox_inches=None, pad_inches=0.0)
    plt.close()
...
if __name__ == '__main__':
    ...
    #plot_goea_top5(celltype, layer)
    plot_goea_top10(celltype, layer)
    #plot_wordcloud(celltype, layer)
    #plot_combined_wordcloud(celltype)
