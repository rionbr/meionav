...
if __name__ == '__main__':
    ...
    # Load Backbone
    rOBfile = '../../04-network/results/network-closure-ortho/spermatocyte/net-closure-ortho-spermatocyte-thr-0p5-HS.gpickle'
    wCSVFile = 'results/goea/goea-non-ortho-backbone-{layer:s}.csv.gz'.format(layer=layer)

    OB = nx.read_gpickle(rOBfile)

    #not_ortho_metric_edges = [(i, j) for i, j, d in OB.edges(data=True) if d.get('is_metric_ortho') is None]
    #not_ortho_metric_nodes = set(list(chain(*not_ortho_metric_edges)))
    ...
    all_nodes = set(OB.nodes())
    not_ortho_metric_nodes = all_nodes.difference(is_ortho_metric_nodes)

    genes = set(dfQ.loc[dfQ.index.isin(not_ortho_metric_nodes), 'UniProtKB/Swiss-Prot ID'].tolist())
