...
public class PlotNetMultiModHS {
    ...
    public static void main(String[] args) throws IOException {
        ...
        */
        ...
        for(Node node : graphModel.getGraph().getNodes()) {
            ...
            // Highlight node based on specific columns
            node.setColor(Color.decode("#000000"));

            // M1-Uniquination
            if (node.getAttribute("d18") == Boolean.TRUE) {
                node.setColor(Color.decode("#aec7e8"));
            }
            // M2-Translation
            if (node.getAttribute("d17") == Boolean.TRUE) {
                node.setColor(Color.decode("#ffbb78"));
            }
            // M3-Splicing
            if (node.getAttribute("d15") == Boolean.TRUE) {
                node.setColor(Color.decode("#98df8a"));
            }
            // M4-rRNA processing
            if (node.getAttribute("d16") == Boolean.TRUE) {
                node.setColor(Color.decode("#c5b0d5"));
            }
            // M5-Protein catabolism
            if (node.getAttribute("d21") == Boolean.TRUE) {
                node.setColor(Color.decode("#f7b6d2"));
            }
            // M6-Mitochondrial translation
            if (node.getAttribute("d9") == Boolean.TRUE) {
                node.setColor(Color.decode("#cedb9c"));
            }
            // M7-Vesicle transport
            if (node.getAttribute("d23") == Boolean.TRUE) {
                node.setColor(Color.decode("#9edae5"));
            }
            // M8-Cell division
            if (node.getAttribute("d14") == Boolean.TRUE) {
                node.setColor(Color.decode("#ff9896"));
            }
            // M9-DNA repair
            if (node.getAttribute("d22") == Boolean.TRUE) {
                node.setColor(Color.decode("#c49c94"));
            }
            // M10-Chromosome segregation
            /*if (node.getAttribute("d28") == Boolean.TRUE) {
                node.setColor(Color.decode("#FFC83B"));
            }*/
            // M11-Carbohydrate catabolism
            /*if (node.getAttribute("d19") == Boolean.TRUE) {
                node.setColor(Color.decode("#FF452D"));
            }*/
            // M12-Spermatid development
            if (node.getAttribute("d20") == Boolean.TRUE) {
                node.setColor(Color.decode("#6971E4"));
            }
