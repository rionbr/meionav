...
# Date: Jul 24, 2019
#
# Description: Plots MM and MM genes that are differentially expressed, calculated by 1_calc_diff_gene_exp.R
#
# Instructions:
...
if __name__ == '__main__':
    ...
    ax1.set_title('MM Library Euclidean Distance')
    ...
    plt.tight_layout()
    #plt.subplots_adjust(left=0.07, right=0.98, bottom=0.06, top=0.95, wspace=0.32, MMpace=0.35)
    plt.savefig('images/img-MM-dist.pdf', dpi=150, bbox_inches=None, pad_inches=0.0)
    plt.close()
    ...
    print('> to DataFrame')
    dfPCA = pd.DataFrame(res[:, 0:5], columns=['1c', '2c', '3c', '4c', '5c'], index=dfC.index)
    ...
    fig, ((ax1,ax2,ax3),(ax4,ax5,ax6)) = plt.subplots(figsize=(12,8),  nrows=2, ncols=3)
    ...
    for dim, ax in zip( range(1,6), [ax2,ax3,ax4,ax5] ):
        ...
        #else:
        ax.scatter(xs, ys, c=dfPCA['color'], marker='o', edgecolor='black', lw=0.5, s=30, zorder=5, rasterized=True)
    ...
    plt.savefig('images/img-MM-pca.pdf', dpi=150, bbox_inches=None, pad_inches=0.0)
    plt.close()
